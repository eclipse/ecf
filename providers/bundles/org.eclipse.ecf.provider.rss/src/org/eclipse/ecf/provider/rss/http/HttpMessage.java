...
/**
 * A HTTP message. It can be read from an InputStream or constructed from scratch.
 * The body of the message is only read on demand and cached in a byte[] for later use.
 * Provides functions for getting and setting headers
 *
 * @author Sergey Yakovlev
 */
public class HttpMessage {
	
	public static final Trace trace = Trace.create("httpMessage");
    ...
    /**
    * Reads the head of the message (startline and headers) from the InputStream
    */
    ...
    /**
    * retrieves the body of the message as a byte[]. If the body is not read in yet,
    * it is read from the InputStream and cached in an internal array
    */
    ...
    */
    ...
    /**
    * writes the message (startline, headers, body) out to the specified OutputStream
    */
    ...
    * content-length is determined according to RFC 2068 page 31
    * 1.) if a body must not be present (hasBody == false), ignore body content.
    * 2.) if chunked transfer-ecoding, this defines the length
    * 3.) content-length header
    * 4.) multipart/byteranges -> ignore, we can't handle them (todo!!)
    ...
    private void readBody(OutputStream out) throws IOException {
        if(hasBody && body == null) {
            if((containsHeader("Transfer-Encoding")) && (getHeader("Transfer-Encoding").equals("chunked"))) {
                ...
                // forwarding a message via a MIME-compliant protocol (RFC 2068, sec.19.4.6).
