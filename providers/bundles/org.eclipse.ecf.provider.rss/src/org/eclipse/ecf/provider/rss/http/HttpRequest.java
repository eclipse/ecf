...
public class HttpRequest extends HttpMessage {
    ...
    /**
    * generate a Request Message which will be read from the specified InputStream
    */
    ...
    * and parses the startline for command, url and protocol.
    */
...
/*
    ...
    public boolean isIEBrowser() {
        if (containsHeader("user-agent")) {
            if (getHeader("user-agent").indexOf("MSIE 4") > -1) {
                return true;
            } else if(getHeader("user-agent").indexOf("MSIE 5") > -1) {
                return true;
            } else if(getHeader("user-agent").indexOf("MSIE 6") > -1) {
                return true;
            }
        }
        return false;
    ...
    public boolean isNNBrowser() {
        if (containsHeader("user-agent")) {
            if (getHeader("user-agent").indexOf("Mozilla/4") > -1) {
                return true;
            } else if(getHeader("user-agent").indexOf("Mozilla/5") > -1) {
                return true;
            }
        }
        return false;
