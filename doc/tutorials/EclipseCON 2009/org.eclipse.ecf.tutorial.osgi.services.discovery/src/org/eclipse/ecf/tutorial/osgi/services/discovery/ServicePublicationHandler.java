...
public class ServicePublicationHandler implements ServiceTrackerCustomizer, Discovery {
	...
	private final IServiceListener serviceListener = new IServiceListener() {
		...
		public void serviceDiscovered(IServiceEvent anEvent) {
			...
			if (checkValid(serviceID)) {
				...
				for (int i = 0; i < discoveredTrackers.length; i++) {
					discoveredTrackers[i].serviceChanged(new DiscoveredServiceNotificationImpl(
		...
		public void serviceUndiscovered(IServiceEvent anEvent) {
			...
			if (checkValid(serviceID)) {
				...
				for (int i = 0; i < discoveredTrackers.length; i++) {
					discoveredTrackers[i].serviceChanged(new DiscoveredServiceNotificationImpl(
							DiscoveredServiceNotification.UNAVAILABLE, serviceInfo));
				}
			}
		}
	};
	...
	private boolean matchWithDiscoveredServiceInfo(ServiceReference serviceReference, IServiceInfo serviceInfo) {
		//TODO tutorial "implement servicePublicationProperties to IServiceType conversion"
		//throw new UnsupportedOperationException("matchWithDiscoveredServiceInfo(ServiceReference serviceReference, IServiceInfo serviceInfo)");
	...
	private boolean checkValid(ServiceReference reference) {
		...
		if (svcInterfaces == null) {
			logError("handleServicePublication", //$NON-NLS-1$
					"ignoring " //$NON-NLS-1$
							+ reference + ". ServicePublication.PROP_KEY_SERVICE_INTERFACE_NAME not set", //$NON-NLS-1$
					null);
			return false;
		}
		...
		if (servicePublicationServiceProperties == null) {
			logError("handleServicePublication", //$NON-NLS-1$
					"ignoring " //$NON-NLS-1$
							+ reference + ". ServicePublication.PROP_KEY_SERVICE_PROPERTIES not set", //$NON-NLS-1$
					null);
			return false;
		}
		...
		if (endpointContainerID == null) {
			logError("handleServicePublication", //$NON-NLS-1$
					"ignoring " //$NON-NLS-1$
							+ reference + ". ECFServicePublication.PROP_KEY_ENDPOINT_CONTAINERID not set", //$NON-NLS-1$
					null);
			return false;
		}
	...
	private void addPropertiesToDiscoveryServiceProperties(IServiceProperties discoveryServiceProperties,
