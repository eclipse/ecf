...
public class EnhancedDebugger implements SmackDebugger {
	...
	private void addBasicPanels() {
		final JSplitPane allPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
		...
		messagesTable = new DefaultTableModel(new Object[] {"Hide", "Timestamp", "", "", "Message", "Id", "Type", "To", "From"}, 0) {
			/**
					 * 
					 */
			private static final long serialVersionUID = -1168247942976999947L;
		...
		final JTable table = new JTable(messagesTable);
		...
		final SelectionListener selectionListener = new SelectionListener(table);
		...
		menuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!"".equals(adhocMessages.getText())) {
					final AdHocPacket packetToSend = new AdHocPacket(adhocMessages.getText());
	...
	private void addInformationPanel() {
		...
		final JPanel connPanel = new JPanel();
		...
		final JPanel packetsPanel = new JPanel();
		packetsPanel.setLayout(new GridLayout(1, 1));
		packetsPanel.setBorder(BorderFactory.createTitledBorder("Transmitted Packets"));

		statisticsTable = new DefaultTableModel(new Object[][] { {"IQ", new Integer(0), new Integer(0)}, {"Message", new Integer(0), new Integer(0)}, {"Presence", new Integer(0), new Integer(0)}, {"Other", new Integer(0), new Integer(0)}, {"Total", new Integer(0), new Integer(0)}}, new Object[] {"Type", "Received", "Sent"}) {
			/**
			 * 
			 */
			private static final long serialVersionUID = -5266697231624638254L;

			public boolean isCellEditable(int rowIndex, int mColIndex) {
				return false;
			}
		};
		final JTable table = new JTable(statisticsTable);
		// Allow only single a selection
		table.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	private String formatXML(String str) {
		try {
			...
			final StreamSource source = new StreamSource(new StringReader(str));
