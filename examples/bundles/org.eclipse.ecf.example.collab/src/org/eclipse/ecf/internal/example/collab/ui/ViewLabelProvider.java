...
class ViewLabelProvider extends LabelProvider {
	public Image getImage(Object obj) {
		Image image = null; // By default, no image exists for sharedObject, but
							// if found to be a specific instance, load from
							// plugin repository.
		...
		if (obj instanceof TreeUser) {
			image = registry.get(ClientPluginConstants.DECORATION_USER);
				...
				&& (((TreeParent) obj).getTreeItem() != null)) {

			String childName = ((TreeParent) obj).getTreeItem().getLabel();
			// TODO: Come up with a better strategy of matching node type to
			// entity decoration.
