...
public class ChatRoomView extends ViewPart implements
	...
	public static final String VIEW_ID = "org.eclipse.ecf.presence.ui.chatroom.ChatRoomView"; //$NON-NLS-1$
	...
	protected static final String VIEW_PREFIX = Messages.ChatRoomView_VIEW_NAME_PREFIX;

	protected static final String DEFAULT_TIME_FORMAT = Messages.ChatRoomView_TIME_FORMAT;

	protected static final String DEFAULT_DATE_FORMAT = Messages.ChatRoomView_DATE_FORMAT;
	...
	protected void handleTextInput(String text) {
		if (messageSender == null) {
			MessageDialog.openError(getViewSite().getShell(), Messages.ChatRoomView_NOT_CONNECTED_TITLE,
					Messages.ChatRoomView_NOT_CONNECTED_MESSAGE);
			return;
		} else {
			try {
	...
	public void initialize(final IChatRoomViewCloseListener parent,
		...
		Display.getDefault().syncExec(new Runnable() {
			public void run() {
				...
				ChatRoomView.this.setTitleToolTip(Messages.ChatRoomView_TITLE_TOOLTiP_PREFIX
						+ roomID.getName() + ", Description: " //$NON-NLS-1$
	...
	protected void addParticipant(IUser p) {
		if (p != null) {
			ID id = p.getID();
			if (id != null) {
				appendText(new ChatLine(NLS.bind(Messages.ChatRoomManagerView_ENTERED_MESSAGE,getDateTime(),
						trimUserID(id)), null));
				memberViewer.add(p);
			}
		}
	...
	protected void removeParticipant(IUser p) {
		if (p != null) {
			ID id = p.getID();
			if (id != null) {
				appendText(new ChatLine(NLS.bind(Messages.ChatRoomManagerView_LEFT_MESSAGE, getDateTime(),trimUserID(id)),null));
	...
	protected void appendText(ChatLine text) {
		...
		if (text.getOriginator() != null) {
			sb.append(NLS.bind(Messages.ChatRoomManagerView_MESSAGE_DATE, getDateTime()));
