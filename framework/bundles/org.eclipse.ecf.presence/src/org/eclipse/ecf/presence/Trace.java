...
public class Trace {
	...
	static {
		try {
			...
			String bundleName = PresencePlugin.getDefault().getBundle()
					.getSymbolicName();
			...
			if (val != null) {
				...
				System.out
						.println("WARNING:  Eclipse platform not being use for trace...overridden by system property org.eclipse.ecf.ui.Trace");
			} else {
				isEclipse = true;
			...
			try {
				String val = System
						.getProperty("org.eclipse.ecf.presence.Trace");
				if (val != null) {
					setTrace(true);
					isEclipse = false;
					// No eclipse Platform available
					System.out
							.println("WARNING:  Eclipse platform not available for trace...using system.out for org.eclipse.ecf.ui");
				} else {
