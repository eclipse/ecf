...
class StandaloneContainerManager implements Observer {
	static Debug myDebug = Debug.create(StandaloneContainerManager.class.getName());
	...
	StandaloneContainerManager(StandaloneContainer space, Item ourMember) {
		container = space;
	...
	void destroySharedObjects(Item m, boolean match) {
		...
		while (i.hasNext()) {
			...
			if (isLoading(removeID)) {
				...
				container.removeSharedObject(removeID);
			}
		}
	}
